                ...
                return (
                  <div key={index} className="flex flex-col gap-2.5">
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-4 md:px-5 py-3 md:py-[14px] rounded-[10px] transition-colors duration-200 cursor-pointer ${isOpen ? 'bg-[#FF850B]' : 'bg-transparent outline outline-1 outline-[#F1F1F1]'
                        }`}
                    >
                      <span className={`text-[13px] md:text-[15px] font-semibold text-left ${isOpen ? 'text-white' : 'text-[#1E1E1E]'
                        }`}>
                        {faq.question}
                      </span>

                      <span className={`flex-shrink-0 w-[26px] h-[26px] md:w-[30px] md:h-[30px] rounded-[5px] flex items-center justify-center ${isOpen ? 'bg-white' : 'bg-[#FF850B]'
                        }`}>
                        {isOpen ? (
